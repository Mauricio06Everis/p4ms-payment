...
import com.example.mspayment.models.dto.PurseDTO;
...
public class PaymentHandler {
    ...
    public Mono<ServerResponse> makePaymentPurse(ServerRequest request) {
        Mono<Payment> payment = request.bodyToMono(Payment.class);
        return payment.flatMap(paymentService::create)
                .flatMap(p -> {
                    kafkaTemplate.send("TEST_TOPIC","Purse", new PurseDTO(p.getPurse().getPhoneTransmitter(), p.getPurse().getPhoneReceiver(), p.getAmount()));
                    return ServerResponse.created(URI.create("/payment/".concat(p.getId())))
                        .contentType(MediaType.APPLICATION_JSON)
                        .bodyValue(p);
                })
                .onErrorResume(error -> Mono.error(new RuntimeException(error.getMessage())));
